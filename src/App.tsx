...
async function getClientforWalletMnemonic(endpoint: string = DEFAULT_ENDPOINT, mnemonic: string = def_mnemonic) {
...
async function makeBankSendTx(sendAddress: string, receivingAddress:string, amount:number, denom:string, chain:string){
...
async function makeIntertxSendTx(sendAddress:string, receivingAddress:string, amount:number, denom:string, gas:number = 90000){
  const client = await getClientforWalletMnemonic();
  const transferAmount = coins(amount, denom)
  const message = {
