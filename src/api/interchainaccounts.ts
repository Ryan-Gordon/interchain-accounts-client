...
export async function getClientforWalletMnemonic(
  ...
  try {
    ...
    console.log("Made connection to client ");
  }
}
export async function makeRegisterTx(addressToRegister?: string) {
  const client = await getClientforWalletMnemonic();
  const myAddress = addressToRegister ? addressToRegister : COSM_DEV_ADDRESS_1;
  const message = {
...
export async function makeBankSendTx(
  sendAddress: string,
...
export async function makeIntertxSendTx(
  sendAddress: string,
  receivingAddress: string,
  amount: number,
  denom: string,
